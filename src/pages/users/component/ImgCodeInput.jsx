...
function CodeInput({ FormItem, Input }) {
  const [codeArr, setCodeArr] = React.useState([]);
  const [loginRule, setLoginRule] = useState({});
  ...
  useEffect(() => {
    setLoginRule(getLoginRule(codeArr));
  }, [codeArr]);
  /**
   * 绘制验证码
   */
  ...
  return (
    <>
      <FormItem name="imgCode" rules={loginRule.imgCodeRule} hasFeedback>
        <Input
